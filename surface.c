...
void surface_scan(system_t *system, double xi, double yi, double zi, double xf, double yf, double zf, double dx, double dy, double dz, double dtheta, double dphi) {

	molecule_t *molecule_backup, *mol_ptr;
			...
			for(z = zi; z <= zf; z += dz) {
				...
				for(theta = 0.0; theta <= M_PI; theta += dtheta) {
					for(phi = 0.0; phi <= 2.0*M_PI; phi += dphi) {
						...
						/* c.o.m. coordinates */
						for(q = 0; q < 3; q++) mol_ptr->com[q] = molecule_backup->com[q];

						/* now move the molecule to the proper point and get the energy */
						energy = surface_point(system, x, y, z, theta, phi);
...
double surface_point(system_t *system, double x, double y, double z, double theta, double phi) {
